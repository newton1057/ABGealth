...
import 'substances_check_screen.dart';

class WaterCheckScreen extends StatefulWidget {
  const WaterCheckScreen({super.key, required this.onContinue});

  final VoidCallback onContinue;

  @override
  State<WaterCheckScreen> createState() => _WaterCheckScreenState();
}

class _WaterCheckScreenState extends State<WaterCheckScreen>
    with TickerProviderStateMixin {
  int? _selectedHydration;
  ...
  static const _sky = Color(0xFF60A5FA);
  static const _background = Color(0xFFF8F5F2);

  final List<WaterOption> _hydrationOptions = [
    WaterOption(
      title: 'Casi nada',
      emoji: '💧',
    ...
    WaterOption(
      title: 'Algo, pero podría ser mejor',
      emoji: '💧💧',
  ...
  void _selectHydration(int index) {
    setState(() {
      _selectedHydration = index;
    });
  }
            ...
            children: [
              ...
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ..._hydrationOptions.asMap().entries.map((entry) {
                        final index = entry.key;
                        final option = entry.value;
                        final isSelected = _selectedHydration == index;
                        return Padding(
                          padding: const EdgeInsets.only(bottom: 12),
                          child: _buildHydrationCard(option, index, isSelected),
                        );
                      }),
      ...
      children: [
        ...
        Container(
          ...
          child: Container(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [AppColors.primary, _blue, _sky],
              ),
              borderRadius: BorderRadius.circular(3),
            ),
          ),
  ...
  Widget _buildContinueButton() {
    final hasSelection = _selectedHydration != null;
    return GestureDetector(
      onTap: hasSelection
          ? () {
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: (_) => SubstancesCheckScreen(
